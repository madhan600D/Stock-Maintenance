...
import {ObjPublicConfigVariables} from '../Declarations/Declarations.js'
...
export class GroupMail{
    ...
    PushGroupMail = async () => {
        try {
            ...
            for(let Index = 0 ; Index <= this.ParentMailOptions.length - 1 ; Index += 1){
                ...
                if(!IsSuccess?.success){
                    ...
                    ObjNotificationKafkaProducer.ProduceEvent('GroupMailSent' , 'user.group_mail.response',KafkaResponse.Data)
                }
            }
        KafkaResponse.Event = 'GroupMailSent'
        ...
        ObjNotificationKafkaProducer.ProduceEvent('GroupMailSent' , 'user.group_mail.response',KafkaResponse.Data)
            ...
            return {success:false , message:"Server side error at PushGroupMail class"}
        }
    ...
    SendMail = async () => {
        try {
            ...
            // Push Mail
            const info = await transporter.sendMail(this.ParentMailOptions[Index]);
            ...
            if (info) {
                ...
                KafkaResponse.Event = 'GroupMailSent'
                KafkaResponse.Data = {Success : false , UserID : this.UserData.UserID , UserName : this.UserData.UserName , UserMail:this.UserData.UserMail}
                ObjNotificationKafkaProducer.ProduceEvent('GroupMailSent' , 'user.create_user.response' ,KafkaResponse )
                DeliveryStatus = await objNotificationDB.NotificationDeliveryStatus.update(
